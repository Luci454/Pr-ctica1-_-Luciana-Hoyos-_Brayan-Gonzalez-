...
public class TyreOff extends PitCrewTeam implements TeamDriverB{
    ...
    //Constructors
    public TyreOff(String name, String rolName, String timeIntoTheCrew, char Boss){
        super(name, rolName, timeIntoTheCrew, Boss);
    }
    public TyreOff(PitCrewTeam Member){
    ...
    private int TyreOffAction(){
        int complete = 0;
        if (energy > 0){
            this.energy = energy-25;
            System.out.println("\nThe Tyre Off remove the tyre.");
            complete = 1;
        }else{
            System.out.println("\nThe Tyre Off is  already tired.");
            complete = 0;
        }
        return complete;
    }
