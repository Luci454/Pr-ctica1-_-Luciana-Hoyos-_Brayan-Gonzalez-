...
public class TyreOn extends PitCrewTeam implements TeamDriverA{
    ...
    //Constructor
    public TyreOn(String name, String rolName, String timeIntoTheCrew, char Boss){
        super(name, rolName, timeIntoTheCrew, Boss);
    }
    public TyreOn(PitCrewTeam Member){
    ...
    private int TyreOnAction(){
        int complete = 0;
        if (energy > 0){
            this.energy = energy-25;
            System.out.println("\nThe Tyre On put the tyre.");
            complete = 1;
        }else{
            System.out.println("\nThe Tyre On is  already tired.");
            complete = 0;
        }
        return complete;
    }
